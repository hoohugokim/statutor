...
## D-0024 — No mandatory project layout; init profiles scaffold conventional directories only
**Status:** accepted
**Context:** `statutor init` produced one fixed skeleton for every ecosystem. A type-aware layout was proposed (plans/v0.6-init-profiles.md, T-0043); mandating one would fight ecosystem conventions and require policy-schema plus Rust conformance work for unproven value, while per-type content templates would presume taste the minimal-constitution rule forbids.
**Decision:** Statutor never mandates a layout. `init` selects a profile — `--type NAME`, else `STATUTOR_INIT_TYPE`, else the first marker hit in the order python (`pyproject.toml`, `setup.py`, `setup.cfg`) > rust (`Cargo.toml`) > node (`package.json`) > docs (`mkdocs.yml`, `_quarto.yml`, existing `docs/`), else `min` — and a profile adds only empty conventional directories (`tests/`, `test/`, `docs/`) on top of the governed files, `plans/archive/`, `notes/`, and the CLAUDE bridge. `none` adds no directory at all. Every create is additive and idempotent: present paths, including files and symlinks, are skipped, never overwritten or adopted. Profiles never write policy, constitution content, or ecosystem files.
**Decision:** Definitions are embedded in the kernel beside TEMPLATES (D-0003: no `profiles/` directory may exist). The `min` fallback stays byte-identical to the pre-v0.6 `init`. Only the CLI reads the environment; in-process callers stay deterministic. Policy schema, hooks, git floor, doctor, and the Rust twin are untouched; `init` remains Python-only.
**Consequences:** T-0043 ships the profiles, the CLI/env parsing, and the matrix tests. A `global.json` init default, team presets, per-type content templates, and doctor warnings for missing directories are deferred, and each needs its own decision.
