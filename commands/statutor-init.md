...
1. Run `python3 ${CLAUDE_PLUGIN_ROOT}/core/statutor_core.py init .` — it creates the governed files, `.statutor.yaml`, `plans/archive/`, `notes/`, and a `CLAUDE.md` importing `@AGENTS.md`, skipping anything that exists. It detects a profile from project markers (python/rust/node/docs) and adds only that ecosystem's empty `tests/`, `test/`, or `docs/` directory; `--type none` (or `min`) opts out per run and `STATUTOR_INIT_TYPE` per user. Profiles never write policy or content (D-0024).
